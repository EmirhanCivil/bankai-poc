...
async function customFetch(url, options) {
  // Rewrite localhost:8443 to host.docker.internal:8443 for container-to-host access
  const rewritten = new URL(url.toString().replace('localhost:8443', 'host.docker.internal:8443'));
  url = rewritten;
...
class CustomOpenIDStrategy extends OpenIDStrategy {
  ...
  authorizationRequestParams(req, options) {
    ...
    // Her login'de Keycloak'a kimlik bilgisi sormaya zorla
